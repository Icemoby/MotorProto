...
display('GEOMETRY_TEST_SCRIPT');
tic

%% Test multiple line object intersections
...
p    = [1 -1];
r    = pi*6/5;
g(1) = Geometry1D.draw('Polyline','Points',y*[0 0;1 1]+x,'Position',p/2,'Rotation',r);
g(2) = Geometry1D.draw('Polyline','Points',y*[0 1;1 0]+x,'Position',p/2,'Rotation',r);
g(3) = Geometry1D.draw('Polyline','Points',y*[0.25 0;0.25 1]+x,'Position',p/2,'Rotation',r);
g(4) = Geometry1D.draw('Polyline','Points',y*[0.75 0;0.75 1]+x,'Position',p/2,'Rotation',r);
g(5) = Geometry1D.draw('Polyline','Points',y*[0 0.25;1 0.25]+x,'Position',p/2,'Rotation',r);
g(6) = Geometry1D.draw('Polyline','Points',y*[0 0.75;1 0.75]+x,'Position',p/2,'Rotation',r);

[X,Y,S] = intersection(g);

assert(numel(X) == 5);
assert(numel(Y) == 5);
assert(numel(S) == 6);
assert(all(cellfun(@(x)(numel(x)),S) == [3 3 2 2 2 2].'));

%Arcs
rad  = 1;
a    = pi*9/10;
g(7) = Geometry1D.draw('Arc','Radius',rad,'Angle',pi+a,'Position',p,'Rotation',pi/3);
g(8) = Geometry1D.draw('Arc','Radius',rad,'Angle',pi+a,'Position',p,'Rotation',-pi/3);
g(9) = Geometry1D.draw('Arc','Radius',rad,'Angle',pi+a,'Position',p+[cos(pi/3/2),sin(pi/3/2)],'Rotation',r);
g(10) = Geometry1D.draw('Arc','Radius',rad,'Angle',pi+a,'Position',p+[cos(-pi/3/2),sin(-pi/3/2)],'Rotation',-r);

[X,Y,S] = intersection(g(7:10));
assert(numel(X) == 9);
assert(numel(Y) == 9);
...
%% Test rotating 1D objects
g1 = Geometry1D.draw('Polyline','Points',[0 0;1 0]);
assert(abs(g1.x(0)) < sqrt(eps));
assert(abs(g1.x(1)-1) < sqrt(eps));
assert(abs(g1.y(0)) < sqrt(eps));
assert(abs(g1.y(1)) < sqrt(eps));

g2 = rotate(copy(g1),pi/2,[0 0]);
assert(abs(g2.x(0)) < sqrt(eps));
assert(abs(g2.x(1)) < sqrt(eps));
assert(abs(g2.y(0)) < sqrt(eps));
assert(abs(g2.y(1)-1) < sqrt(eps));

g3 = Geometry1D.draw('Polyline','Points',[0 0;1 0],'Position',[1 0]);
assert(abs(g3.x(0)-1) < sqrt(eps));
assert(abs(g3.x(1)-2) < sqrt(eps));
assert(abs(g3.y(0)) < sqrt(eps));
assert(abs(g3.y(1)) < sqrt(eps));

g4 = rotate(copy(g3),-pi/2,[0 0]);
assert(abs(g4.x(0)) < sqrt(eps));
assert(abs(g4.x(1)) < sqrt(eps));
assert(abs(g4.y(0)+1) < sqrt(eps));
assert(abs(g4.y(1)+2) < sqrt(eps));


%% Test Unions
rotation = 2*pi*rand;
position = 5-10*rand(1,2);
g1 = Geometry2D.draw('Rectangle','Length',1,'Width',1,'Position',position,'Rotation',rotation);
g2 = Geometry2D.draw('Rectangle','Length',0.5,'Width',0.5,'Position',position,'Rotation',rotation);
g3 = Geometry2D.draw('Rectangle','Length',1,'Width',1,'Position',[cos(rotation) sin(rotation)]+position,'Rotation',rotation);
g4 = Geometry2D.draw('Rectangle','Length',0.5,'Width',0.5,'Position',0.75*[cos(rotation) sin(rotation)]+position,'Rotation',rotation);
g5 = Geometry2D.draw('Rectangle','Length',0.5,'Width',0.5,'Position',0.75*[sin(rotation) -cos(rotation)]+position,'Rotation',rotation);

s1 = Union2D([g1 g2]);
assert(numel(s1.Curves) == 4);

s2 = Union2D([g1 g3]);
assert(numel(s2.Curves) == 6);

s3 = Union2D([g1 g4]);
assert(numel(s3.Curves) == 8);

s4 = Union2D([g2 g3]);
assert(numel(s4.Curves) == 8);

s5 = Union2D([g2 g4]);
assert(numel(s5.Curves) == 8);

s6 = Union2D([g3 g4]);
assert(numel(s6.Curves) == 6);

s7 = Union2D([g1 g2 g3 g4 g5]);
assert(numel(s7.Curves) == 10);

%Test with shapes that intersect at points
g1 = Geometry2D.draw('Rectangle','Length',1,'Width',1,'Position',[1.5 0]);
g2 = Geometry2D.draw('Sector','Angle',pi,'Rotation',-pi/2,'Radius',[1 1.5]);
...
g2 = Geometry2D.draw('Rectangle','Length',0.5,'Width',0.5,'Position',[0.25,0]);
d1 = g1-g2;
assert(numel(d1.Curves) == 8);
assert(abs(d1.area - 0.75) < sqrt(eps));

null = d1*g2;
assert(numel(null.Curves) == 0);
assert(null.area == 0);
...
assert(all(g1(1).Position == [0 0]));
assert(abs(g1(1).area - 1) < sqrt(eps));

assert(all(g1(2).Position == [0 -2]));
...
s = Geometry2D.draw('Sector','Radius',[0.05 0.085],'Angle',pi/8,'Rotation',-pi/8);
r = Geometry2D.draw('Rectangle','Position',[0.075 0],'Length',0.005,'Width',0.025);
