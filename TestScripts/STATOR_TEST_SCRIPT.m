...
Stator = Model.newComponent('myStator','StatorLamination');

assert(Stator == Model.Components(1));
assert(strcmp(class(Model.Components(1)),'StatorLamination'));
assert(strcmp(Model.Components(1).Name,'myStator'));
...
assert(stator.Regions(2).Source    == 1);
assert(stator.Regions(4).Source    == 1);
assert(stator.Regions(6).Source    == 1);

assert(stator.Regions(8).Source    == 2);
assert(stator.Regions(10).Source   == 2);
assert(stator.Regions(12).Source   == 2);

assert(stator.Regions(8).Polarity  == -1);
assert(stator.Regions(10).Polarity == -1);
assert(stator.Regions(12).Polarity == -1);

assert(stator.Regions(14).Source   == 3);
assert(stator.Regions(16).Source   == 3);
assert(stator.Regions(18).Source   == 3);

assert(all(stator.Sources(1).Regions  == [2 4 6]));
assert(all(stator.Sources(1).Polarity == [1 1 1]));

assert(all(stator.Sources(2).Regions  == [8 10 12]));
assert(all(stator.Sources(2).Polarity == [-1 -1 -1]));

assert(all(stator.Sources(2).Regions  == [14 16 18]));
assert(all(stator.Sources(2).Polarity == [1 1 1]));
