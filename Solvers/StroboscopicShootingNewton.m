classdef StroboscopicShootingNewton < Solver
...
%}
    ...
    methods
        %% Constructor
        function this = StroboscopicShootingNewton(varargin)
            if nargin > 0
                for iArg = 1:2:nargin
        ...
        function solution = solve(this, model, x0)
            ...
            %% Initialize
            tTimes     = matrixFactory.getTimePoints(this.TimePoints.Value);
            times      = tTimes;
            T1         = times(end);
            T2         = T1 * q / p;
            k          = zeros(1,N);
            tau_k      = zeros(1,N);
            k(1)       = numel(times);
            tau_k(1)   = mod(T1,T2) / T2;
            for i = 2:N
                times    = [times (times(end)+tTimes(2:end))];
                k(i)     = numel(times);
                tau_k(i) = mod(i*T1,T2) / T2;
            end
            ...
            while shootingErr > shootingTol && iShooting <= maxShootingItt
                ...
                r  = (8*x{k(1)}-7*x{k(2)}+2*x{k(3)})/3 - x{1};
                sf = sum(abs(J), 2);
                ...
                if shootingErr > shootingTol && maxGMRESItt > 0
                    if storeLDL
                        f = @(y)(StroboscopicShootingNewton.NSTFJMVP(v,times,k,matrixFactory,A,b,c,y,L,D,P,S,R));
                        [dx,~,~,~] = gmres(f, r, maxGMRESItt, gmresTol, 1, [], [], r);
                    else
                        f = @(y)(StroboscopicShootingNewton.NSTFJMVP(v,times,k,matrixFactory,A,b,c,y));
                        [dx,~,~,itter] = gmres(f, r, maxGMRESItt, gmresTol, 1, [], [], r);
                        iLinear = iLinear + itter(1) * itter(2) * nStages * nTimes;
                    end
                    x{1} = x{1} - dx;
                    [norm(x{1}) norm(dx)/norm(x{1}) ShootingResidual]
                else
                    %x{1} = x{end};
                end
    ...
    methods (Static)
        function mvp = NSTFJMVP(v,times,k,matrixFactory,A,b,c,r,L,D,P,S,R)
            ...
            mvp = (8*y{k(1)}-7*y{k(2)}+2*y{k(3)})/3 - y{1};
        end
        ...
        function solverOut = configureSolver(varargin)
            if nargin > 0
                solverOut = StroboscopicShootingNewton(varargin{:});
            end
        end
    end
end